...
//
// A stable device UUID persisted in the standard UserDefaults domain
// (device-local, never synchronized). Used for note version lineage
...
public enum AppDevice {
    ...
    public static func current(defaults: UserDefaults = .standard) -> DeviceIdentity {
