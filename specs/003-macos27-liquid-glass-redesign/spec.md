...
用户在设置中配置通用（含 Dock 开关与笔记字体 Notes 分区）、同步（含人性化错误与高级区）、隐私（懒请求与状态展示）——整体呈现为原生 macOS Settings 窗口而非网页式表单。
...
5. **Given"同步"面板，**When** 用户查看默认页面，**Then** 聚焦用户可理解的概念：同步状态（FR-012 分类映射，不用内部 Configured 状态）、存储位置、上次成功同步时间、自动同步开关、同步频率（001 FR-152 选项保留：仅变更后同步 / 每 5/15/30/60 分钟）、本 Mac 上 vault 是否保持解锁（001 FR-162a）、主同步/重试动作；vault 未解锁时如实显示锁定状态并禁用不可执行控件（2026-08-14 Rev 2）。
...
8. **Given"通用"面板的 Notes 分区，**When** 用户设置"笔记字体"，**Then** 面向用户的概念为主字体 + 系统回退（不暴露实现性排版术语），**And** 提供中英文混排的多语言实时预览，**And** 默认值对齐 macOS 系统排版与回退行为（001 FR-043 行为保留，2026-08-14 Rev 2 由独立面板并入通用面板）。
...
- **FR-053**（同步）: 默认同步页面 MUST 聚焦用户概念：同步状态、存储位置、上次成功同步时间、自动同步开关、同步频率（001 FR-152 选项与默认值不变）、本 Mac 上 vault 解锁状态（001 FR-162a 不变）、主同步/重试动作；错误 MUST 按 FR-012 呈现为可读描述与恢复动作；同步状态 MUST 由 FR-012 分类映射驱动（2026-08-14 Rev 2 修订），MUST NOT 以内部 "Configured" 状态冒充用户状态；vault 未解锁时 MUST 如实显示锁定状态并禁用不可执行控件（解锁入口 MAY 提供）。
- **FR-054**（同步·高级）: "加入既有保险库" MUST 作为首次配置流程的一部分（初始设置），并在高级区以独立入口提供恢复性重入（2026-08-14 Rev 2 修订：Join Another Vault 为独立产品动作，MUST NOT 并入"更改存储位置"流程）；"新建存储位置"（原"替换仓库"）"断开同步"（原"移除配置"）"导出同步配置""导出诊断包"等维护操作 MUST 位于独立"高级"管理区，与常规偏好分离；新建存储位置 MUST 保持 001 FR-154 语义（本地笔记保留、新 vault 全新引导、不自动删除旧仓库远程数据）且确认文案 MUST 明示该语义；破坏性操作 MUST 在视觉与语义上与常规操作区分，并 MUST 要求相应确认；加密密码丢失警告（001 FR-163）MUST 采用标准警告样式与简洁文案（稳定配置态下收敛为 Recovery 信息行），MUST NOT 主导整个面板。
- **FR-055**（通用·Notes）: 通用面板 MUST 面向普通用户提供"笔记字体"概念（主字体 + 系统回退，不使用实现性排版术语）；中英文 MUST 自然混排（001 FR-043 行为不变）；默认值 MUST 对齐 macOS 系统排版与系统回退；MUST 提供有意义的多语言实时预览。
