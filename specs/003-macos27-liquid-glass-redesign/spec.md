...
3. **Given** Library 工具栏，**When** 用户点击"新建笔记"工具栏按钮，**Then** 与使用快捷键（001 FR-120 全局快捷键或 ⌘N）创建的行为一致：新笔记窗口打开并获得键盘焦点（001 FR-007a）。
...
用户在设置中配置通用（含快捷键录制）、同步（含人性化错误与高级区）、字体（多语言预览）、权限（懒请求与状态展示）——整体呈现为原生 macOS Settings 窗口而非网页式表单。

**Why this priority**: 设置是"第一方质感"的可感知组成部分（目标 1/4/5）。同步与权限的正确呈现直接影响信任，快捷键界面影响键盘工作流（目标 7）。

**Independent Test**: 打开设置，验证原生导航与表单规范；在四个面板间切换；录制一个全局快捷键并验证冲突反馈；查看同步状态与高级区；检查字体预览的中英文渲染；检查权限状态展示。

**Acceptance Scenarios**:

1. **Given** 设置窗口，**When** 用户查看导航，**Then** 使用原生 macOS Settings 工具栏式标签导航（替代大尺寸自定义分段控件），**And** 包含"通用 / 同步 / 字体 / 权限"四个逻辑区域。
2. **Given** 任一设置面板，**When** 用户查看布局，**Then** 遵循原生表单惯例（对齐、间距、控件、分隔线、标签、说明文字、分组），**And** 不使用大圆角灰矩形包裹每个分区，**And** 面板高度适配内容（无数百像素的固定空白画布）。
3. **Given"通用"面板，**When** 用户查看，**Then** 保留：显示 Dock 图标，以及全部全局快捷键项（打开/关闭 Library、新建空白笔记、区域截图到新笔记、窗口截图到新笔记、剪贴板新建笔记、搜索全部笔记、显示/隐藏笔记窗口）。
4. **Given"通用"面板的快捷键界面，**When** 用户录制快捷键，**Then** 呈现为原生 macOS 快捷键偏好界面（当前值清晰可见、录制状态明确、可重置、冲突/错误反馈），**And** 冲突检测遵循 001 FR-121（不静默替换既有系统或应用快捷键）。
5. **Given"同步"面板，**When** 用户查看默认页面，**Then** 聚焦用户可理解的概念：同步状态、提供商、上次成功同步时间、自动同步开关、同步频率（001 FR-152 选项保留：仅变更后同步 / 每 5/15/30/60 分钟）、本 Mac 上 vault 是否保持解锁（001 FR-162a）、主同步/重试动作。
6. **Given"同步"面板，**When** 用户查看高级区，**Then** "替换仓库""移除配置""导出同步配置""导出诊断包"等维护操作位于独立的"高级"管理区，**And** 破坏性操作在视觉与语义上与常规操作区分并要求确认（001 FR-154 替换确认语义保留）。
7. **Given"同步"面板，**When** 用户看到加密密码丢失警告，**Then** 采用标准警告样式、语言简洁（001 FR-163 语义保留），**And** 不占据整个面板。
8. **Given"字体"面板，**When** 用户选择"笔记字体"，**Then** 面向用户的概念为主字体 + 系统回退（不暴露实现性排版术语），**And** 提供中英文混排的多语言实时预览，**And** 默认值对齐 macOS 系统排版与回退行为（001 FR-043 行为保留）。
9. **Given"权限"面板，**When** 用户查看，**Then** 每项权限展示：当前状态、为何需要、依赖的功能、打开/发起系统权限流程的动作；未使用的权限不被表述为强制（Constitution VI 修订后语义），**And** 请求时机保持懒请求（001 FR-131/132/133 保留）。
...
- 快捷键录制冲突（系统/其他应用已占用）：清晰报错并保持未录制状态（001 FR-121）。
...
- 设置窗口重设计：原生 Settings 导航与表单、通用（快捷键录制）、同步（状态 + 高级区）、字体（多语言预览）、权限（状态展示）。
...
- 旧数据/配置迁移兼容性验证。
- 全局快捷键"搜索全部笔记"行为补全：`searchAll` 与 `stickynotes://search` 深链打开 Library 并聚焦搜索框（001 FR-120 动作身份不变，D8）。
...
- 修改全局快捷键的绑定集合或动作语义（例外：仅"搜索全部笔记"的打开+聚焦搜索框补全，见 In-Scope 与 FR-072）。
...
- **FR-003**: 搜索 MUST 使用原生 macOS 搜索体验（工具栏搜索栏：系统外观、清空/退出行为、键盘可达），MUST 视觉上属于工具栏（单一控制行，MUST NOT 形成第二工具行）；结果更新时效 MUST 保持 001 FR-024/FR-024a（查询变化 ≤100 ms 开始更新；10,000 条笔记 ≤200 ms 完成）。
...
- **FR-050**: 设置窗口 MUST 采用原生 macOS Settings 工具栏式标签导航（System Settings 风格），替代大尺寸自定义分段导航；逻辑区域 MUST 保持：通用、同步、字体、权限。
- **FR-051**: 设置各面板 MUST 遵循原生表单惯例（对齐、间距、控件、分隔线、标签、说明文字、分组）；MUST NOT 将每个分区放入大圆角灰矩形容器；MUST NOT 使用固定高度的大画布（面板高度适配内容）。
- **FR-052**（通用）: 面板 MUST 保留"显示 Dock 图标"与全部全局快捷键项（打开/关闭 Library、新建空白笔记、区域截图到新笔记、窗口截图到新笔记、剪贴板新建笔记、搜索全部笔记、显示/隐藏笔记窗口）；快捷键界面 MUST 为原生 macOS 快捷键偏好形态：当前值可见、录制状态明确、可重置、冲突/错误反馈（001 FR-121 语义：检测冲突、不静默替换）。
- **FR-053**（同步）: 默认同步页面 MUST 聚焦用户概念：同步状态、提供商、上次成功同步时间、自动同步开关、同步频率（001 FR-152 选项与默认值不变）、本 Mac 上 vault 解锁状态（001 FR-162a 不变）、主同步/重试动作；错误 MUST 按 FR-012 呈现为可读描述与恢复动作。
- **FR-054**（同步·高级）: "加入既有仓库/保险库" MUST 作为首次配置流程的一部分（初始设置），并在高级区提供恢复性重入入口；"替换仓库""移除配置""导出同步配置""导出诊断包"等维护操作 MUST 位于独立"高级"管理区，与常规偏好分离；日常切换仓库 MUST 走"替换仓库"并保持 001 FR-154 语义（本地笔记保留、新 vault 全新引导、不自动删除旧仓库远程数据）；破坏性操作 MUST 在视觉与语义上与常规操作区分，并 MUST 要求相应确认（001 FR-154 替换语义不变）；加密密码丢失警告（001 FR-163）MUST 采用标准警告样式与简洁文案，MUST NOT 主导整个面板。
- **FR-055**（字体）: 面板 MUST 面向普通用户提供"笔记字体"概念（主字体 + 系统回退，不使用实现性排版术语）；中英文 MUST 自然混排（001 FR-043 行为不变）；默认值 MUST 对齐 macOS 系统排版与系统回退；MUST 提供有意义的多语言实时预览。
- **FR-056**（权限）: 面板 MUST 展示每项权限：当前状态、为何需要、依赖的功能、打开/发起系统权限流程的动作；未使用相关功能的权限 MUST NOT 被表述为强制；请求时机 MUST 保持懒请求（001 FR-131/FR-132/FR-133 与 Constitution VI 修订后语义不变）。
...
- **FR-072**: 每个重要工具栏命令 MUST 有对应的菜单栏命令（按 macOS 惯例位于 File/Edit/View/Window/Help）；全局快捷键 MUST 保留并可改进（001 FR-120 动作不变）；常用本地快捷键（⌘N 新建、⌘F 搜索、⌘W 关闭、⌘, 设置、⌘⌫ 移入 Trash 等）MUST 遵循 macOS 标准；MUST NOT 在已有 macOS 标准等价物时发明自定义交互；上下文菜单 MUST 暴露有用的笔记操作（如颜色、置顶、导出、移入 Trash、002/001 既定操作），MUST NOT 复制整个应用菜单。"搜索全部笔记"全局快捷键与 `stickynotes://search` 深链 MUST 打开 Library 并聚焦搜索框（001 FR-120 动作身份不变，D8 补全）。
...
- 快捷键录制冲突：清晰报错并保持未录制状态（001 FR-121）。
...
- **SC-013**: 高级同步维护操作与常规同步偏好分离（"高级"区断言）。
...
- 全局快捷键动作集合不变（001 FR-120）；仅录入界面与呈现重设计（FR-052）；"搜索全部笔记"增加聚焦搜索框的呈现补全（FR-072）。
