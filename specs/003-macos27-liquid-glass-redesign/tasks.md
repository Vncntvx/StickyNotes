...
- [X] T025 [US1] Complete the Search All Notes action (research.md D8): `stickynotes://search` deep link now opens Library and focuses the search field — **2026-08-10 更新**：全局快捷键 `searchAll` 已随 001 FR-120 移除，仅深链承担该动作（001 FR-120 动作身份由深链契约保留）
...
- [X] T041 [P] [US4] Update `AppTests/ShortcutConfigurationTests.swift`: recorder conflict semantics (001 FR-121): conflicts reported clearly, never silently replaced, reset/clear works, fully keyboard-operable — MUST FAIL — **CLOSED 2026-08-10（removed）**：全局快捷键随 001 FR-120/FR-121 移除，`ShortcutConfigurationTests.swift` 已删除，无录制器语义可断言
...
- [X] T045 [US4] Rework General panel: "Show Dock Icon" toggle only — **2026-08-10 更新**：7 个全局快捷键项与原生快捷键偏好形态随 001 FR-120/FR-121 移除（FR-052 保留 Dock 开关部分）
...
- [X] T072 [P] Full regression: `DEVELOPER_DIR=/Applications/Xcode-beta.app/Contents/Developer swift test --package-path Packages/StickyCore` + `xcodebuild test -project StickyNotes.xcodeproj -scheme StickyNotes -destination 'platform=macOS'` — all suites green (StickyCore 92, AppTests 44, AppUITests ≥5 — count grows with new UI tests); regression checklist includes 001 FR-011a resilience: Library open/sort/search failure → no crash, no data loss, non-blocking localized notice (spec §Failure & Recovery); ~~WidgetExtension behavior unchanged (001 FR-110a/111/112)~~ (widget removed 2026-08-13)
- [X] T073 [P] Performance regression (SC-024/FR-091): `PerformanceBaselineTests`/`SearchPerformanceTests`/`KeystrokeLatencyTests` green; 10k-note grid virtualization scrolling + search ≤200 ms verified (FR-094a/FR-072b non-regression, CHK042); no idle CPU from new presentation layer (SC-006); assert search start-update latency (query change → first update ≤100 ms, 001 FR-024a) in SearchPerformanceTests; if micro-timing unsupported, record waiver and spot-check manually
- [X] T074 [P] Manual visual QA matrix per plan.md §12: light/dark, active/inactive, system accent, Reduce Transparency/Motion, Increase Contrast/Show Borders, display scale, 320 pt & wide Library, compact note window, multi-window, keyboard-only, VoiceOver walkthrough; itemize and check the SC-018 Apple HIG walkthrough list (CHK054 — which HIG items, per-surface), cross-window visual-language coherence check (FR-082), and record results in quickstart.md; core capture-loop timing (open Library → create → type → close → reopen → search-find, ≤30 s, 001 SC-011 / SC-023) verified manually
- [X] T075 [P] Localization completeness: zh-Hans/en strings complete for all new UI (banner three elements, palette, menus, settings, confirmations) via `AppTests/LocalizationCompletenessTests.swift`（先核实 001 是否存在；不存在则本任务新建之）+ String Catalog; no internal identifiers in any user-facing string (FR-012)
- [X] T076 [P] Documentation updates: finalize API usage record in `Documentation/toolchain.md` (incl. `visibilityPriority`/`effectIsInteractive` guards); reconcile plan.md/research.md with implementation reality; mark 001 spec FRs superseded: 001 FR-002a→003 FR-020/FR-021; 001 FR-004→003 FR-002/002a/003/004/005/006/007; 001 FR-030a→003 FR-040/FR-041; 001 FR-031→003 FR-044/FR-045; 001 FR-040a→003 FR-030/031/032 (CHK062) so cross-feature traceability stays truthful
- [X] T077 Final verification: SC-001..SC-025 checklist pass; `git diff --exit-code` after `xcodegen generate` (no project drift); feature artifacts (spec/plan/tasks) consistent via `/speckit.analyze`
...
- StickyCore = zero changes (REUSE) — any task touching `Packages/StickyCore/**` violates the plan (WidgetExtension removed 2026-08-13)
...
**Purpose**: `/speckit.converge` 2026-08-09 发现项落地。F1 CRITICAL：菜单栏图标右键/⌥-点击下拉菜单缺失（FR-001/US1-AC7/Constitution X）；F2/F3 HIGH：T074 手动视觉 QA 矩阵与 AppUITests 未执行；F4/F5 LOW：遗留文件清理。StickyCore 仍零改动。

- [X] T078 Implement the menu-bar icon right-click (or ⌥-click) dropdown menu per FR-001/FR-006/US1-AC7/Constitution X: right-click or ⌥-click on the status-item icon MUST present a dropdown menu with 打开 Library / 设置 / 帮助 / 关于 / 退出 (left-click keeps opening the Library window — FR-001 semantics unchanged); wire the existing `openSettings`/`openAbout`/`openHelp` closures (currently declared but never invoked in `App/Sources/Features/Library/MenuBarLibraryScene.swift`) and a Quit action; choose a vehicle consistent with the T018 spike (secondary `NSStatusItem` menu, `rightMouseDown`/⌥-click handling on the MenuBarExtra icon, or SwiftUI `MenuBarExtra` dual-interaction pattern) and record the decision; add the T015-mandated UI test — with Dock icon hidden (LSUIElement), Settings/About/Help/Quit remain reachable via the menu-bar interface (`AppUITests/CriticalFlowsUITests.swift`) — per FR-001/FR-006 (missing) — **VERIFIED 2026-08-09**: vehicle = dedicated `NSStatusItem` status-bar menu registered via `configureMenuBarDropdown()` (AppKit path, consistent with T018 spike's AppKit probing; SwiftUI MenuBarExtra cannot host a second NSMenu on macOS 27 beta); new `App/Sources/Features/Library/MenuBarDropdownMenu.swift` dispatches via `MenuBarDropdownDispatcher` to the existing `openSettings`/`openAbout`/`openHelp` closures + new Quit action (`NSApp.terminate`); left-click toggling of the SwiftUI library window unchanged (FR-001); build green. The T015-mandated UI test is **CANCELLED 2026-08-09 (product decision)**: all AppUITests click journeys removed — only the launch smoke test remains (详情见 001 history/tasks-log.md 2026-08-09 条目)**
- [ ] T079 Execute the manual visual QA matrix per T074/plan.md §12 and record itemized results in `specs/003-macos27-liquid-glass-redesign/quickstart.md` (currently "待人工执行"): light/dark, active/inactive, system accent, Reduce Transparency/Motion, Increase Contrast/Show Borders, display scale, 320 pt & wide Library, compact note window, multi-window, keyboard-only, VoiceOver walkthrough; register SC-018 Apple HIG walkthrough checklist items per surface (CHK054); cross-window visual-language coherence check (FR-082); core capture-loop timing ≤30 s (SC-023) per SC-018/SC-014/SC-015/SC-016 — **DEFERRED 2026-08-09 (documented per T174 option b precedent)**: requires an interactive desktop session / Mac with a display — the headless session cannot present MenuBarExtra windows or drive synthetic interaction; deferral recorded in quickstart.md §实施验证记录**
- [ ] T080 Run the AppUITests suite in an interactive desktop session or CI with a display and fix any failures: `DEVELOPER_DIR=/Applications/Xcode-beta.app/Contents/Developer xcodebuild test -project StickyNotes.xcodeproj -scheme StickyNotes -destination 'platform=macOS'` covering the 14 journeys in `AppUITests/CriticalFlowsUITests.swift` (T015/T016/T035/T040/T053/T060/T061 — toolbar structure, search focus, Trash confirmation, palette, settings tabs, banner, scaling, keyboard-only); all suites green per T072/Constitution XII — **DEFERRED 2026-08-09 (documented per T174 option b precedent)**: the 14 click journeys are **CANCELLED** (product decision — unreliable on macOS 27 beta headless sessions); the remaining launch smoke test requires an interactive desktop session (XCUITest runner cannot initialize headless); deferral recorded in quickstart.md §实施验证记录**
- [X] T081 Remove the dead legacy footer view `App/Sources/Features/Library/SyncStatusView.swift` (zero production references since footer removal, FR-006/T024) and its stale test references in `AppTests/LibraryWindowPresentationTests.swift` (snapshotSyncStatusViewContractIsStable) and stale comments in `AppTests/AsyncFeedbackPolicyTests.swift`/`AppTests/AppLogicTests.swift`; verify all suites stay green; if retention is justified, document the rationale instead per plan §14 REPLACE (unrequested) — **VERIFIED 2026-08-09**: file removed via `git rm`; `snapshotSyncStatusViewContractIsStable` deleted; stale comments in AsyncFeedbackPolicyTests/AppLogicTests rewritten to reference the superseding SyncAttentionBanner (003 FR-010); retention not justified (zero production references); AppTests 143 tests / 32 suites green
- [X] T082 Rename `App/Sources/Features/Library/LibrarySearchView.swift` to `LibraryCardGrid.swift` (it now hosts `LibraryCardGrid` after the native-search replacement, T020) to match plan.md §14 change map, updating any references; run `xcodegen generate` and verify no project drift (T077) per plan §14 — **VERIFIED 2026-08-09**: `git mv` executed; file-header MARK updated; stale `LibrarySearchView` reference in `EmptyStateView.swift` comment updated; `xcodegen generate` regenerated project with only the intended rename in pbxproj (no drift); build-for-testing green (partial)

---

## Phase 12: Settings shell & IA correctness (Rev 2, 2026-08-14 approved plan)

**Purpose**: 执行 2026-08-14 评审通过的 Settings 修复计划 Phase A/B。StickyCore 仅 SystemBridge/SecurityCore 允许的最小变更（Phase B 解锁依赖既有原语；Phase C Keychain 加固另立 PR）。FR-050/051/053/054/055/056 已按 Rev 2 修订（本 spec）。

- [ ] T175 Update Settings policy sources + tests FIRST (Constitution XII): `App/Sources/Features/Settings/SettingsPolicies.swift` (`SettingsLayoutPolicy.logicalAreas` → `["General","Sync","Privacy"]`; remove `fixedCanvasHeight`; add `SettingsWindowPolicy` — defaultSize 640×560 / minimum 600×440 / `windowSizeStableAcrossTabs` / `onlyOverflowingTabsScroll`), `SyncAdvancedAreaPolicy.swift` (Rev 3 operation names: Join Another Vault / Set Up New Storage Location 归入 Storage "Manage…"；Advanced 仅 Export Sync Profile / Export Diagnostic Bundle；Disconnect Sync 独立)；update `AppTests/AppLogicTests.swift` (FR-050), `AppTests/SyncCompositionTests.swift` (FR-054 Rev 3), `AppTests/Phase27LocalizationTests.swift` (renamed strings); add `AppTests/SettingsWindowPolicyTests.swift`; suites MUST FAIL before implementation.
- [ ] T176 Unify Settings window path (FR-051 Rev 2): scene-only entry via captured `@Environment(\.openSettings)` (verified working on macOS 27 beta 2026-08-14 — the earlier `showSettingsWindow:` selector no-op for LSUIElement apps, verified 2026-08-08, does not apply to this mechanism); NSWindow fallback exists ONLY for the degenerate never-captured case, guarded against stacking, and must never coexist with the scene window (the time-based probe was removed 2026-08-14 after it produced two Settings windows when the scene presented slowly); apply stable shell geometry: scene `.defaultSize(width: 640, height: 560)` + `.windowResizability(.contentMinSize)`, root `.frame(minWidth: 600, minHeight: 440, idealWidth: 640, idealHeight: 560)`; fallback window `minSize` 600×440 + `setFrameAutosaveName`; menu bar/app menu/⌘, all route through one entry (`CommandGroup(replacing: .appSettings)` + ⌘, shortcut).
- [ ] T177 Merge Fonts into General (FR-050/FR-055): `SettingsTab` → general/sync/privacy; convert `FontPreferenceView` into `NoteFontSection` under General "Notes" section (storage key + `FontPreferenceUI` constants unchanged).
- [ ] T178 Restructure SyncSettingsView (FR-053/FR-054 Rev 3, 2026-08-14): Status (resolver-driven) / Storage (truncating address + "Manage…" menu hosting Join Another Vault… and Set Up New Storage Location…) / Automatic Sync ("Sync changes automatically" toggle + "Periodic sync" picker with Off) / Security ("Keep sync unlocked on this Mac" + verified Keychain copy + Recovery info row) / Advanced (Vault ID, Export Sync Profile…, Export Diagnostic Bundle… only) / bottom standalone "Disconnect Sync…" destructive entry; unconfigured state: Set Up Sync… / Join Existing Vault…; transient status/error feedback (auto-clear, `AccessibilityAnnouncements.announce`, no persistence across Settings reopen).
- [ ] T179 Sync state correctness (FR-053 Rev 2/Rev 3): `SyncCoordinator.isVaultUnlocked`; wire `SyncStatusResolver` with real locked state; disable Sync Now/remember-unlock when locked（Rev 3：Periodic sync schedule 为设备本地偏好，锁定态仍可配置，不禁用）; minimal `unlock(password:)` (fetch bootstrap by locator → `openRemoteBootstrap` → `wireEngine`, read-only remote); Locked UI + Unlock… action; tests in `AppTests/SyncCoordinatorUnlockTests.swift` (in-memory provider: unlock success / wrong-password fail-closed).
- [ ] T180 Privacy panel (FR-056 Rev 2): remove Accessibility row (no current consumer; constitution VI); Screen Recording single row with two observable states (Granted / Not Granted + Enable… → request → fallback System Settings pane); footer copy updated; remove dead `.denied` UI branches.
- [ ] T181 Localization: add en + zh-Hans entries for all new/changed strings to `App/Resources/Localizable.xcstrings` (FR-180a); verify `AppTests/LocalizationCompletenessTests` + `Phase27LocalizationTests` green.

## Phase 13: UI 语义与信息架构重构批 (Rev 3, 2026-08-14 approved plan)

**Purpose**: 执行 2026-08-14 审计结论驱动的 UI 语义重构批（P0+P1 全量，独立 worktree `feature/ui-semantics-refactor`）。FR-003/003a/053/054/055/055a/072 与 SC-013 已按 Rev 3 修订（本 spec）。每项测试先行（Constitution XII）。

- [ ] T182 Note body font (FR-055/055a Rev 3): `NoteFontSection` 的 TextField/draft/Reset/@FocusState 替换为系统字体族 `Picker(.menu)`（首项 "System Default" tag nil，`NSFontManager.availableFontFamilies` 排序，选择即提交）；预览改多行样本并真实应用 lineSpacing（`TextSpacingPreset.lineSpacingValue` 与 `RichTextView.paragraphStyle` 共用单一来源）；设置文案改名 "Note body font"；卡片正文预览跟随用户字体（`MenuBarLibraryScene` 增 `typography` 参数透传 `NoteCardView.previewFontFamily`，字号与 2 行截断不变，标题保持 .headline）；新增 `NoteFontChoicePresentation` 纯函数与测试（nil↔System Default、排序、存储值缺失保留可选）；更新 `FontPreferenceApplicationTests`/`TypographySettingsUITests`/`Phase27LocalizationTests`。
- [ ] T183 Trash ⋯ 菜单 (FR-014b/026): 删除 `LibraryCardGrid` grid 上方独立 "Empty Trash…" 行（grid 起始位置 Notes/Trash 一致）；`LibraryModel` 增 `emptyTrashConfirmationRequested` + request/acknowledge（仿 searchFocusRequested 模式，`setScope` 离开 trash 复位）；`LibraryCardGrid` 确认条改模型标志驱动、复用 `InlineConfirmationBar` 与 Escape；`MenuBarLibraryScene` headerRow 在 trash scope 显示 `Menu`（"Empty Trash…" `.destructive`，cards 为空禁用，ellipsis label + accessibilityLabel）；测试先行（模型标志生命周期）。
- [ ] T184 NSSearchField bridge (FR-003/003a): 新增 `App/Sources/Features/Library/LibrarySearchField.swift`（NSViewRepresentable + NSSearchFieldDelegate，`controlTextDidChange` 写 `$model.searchQuery`，`focusRequested` 翻转 → `makeFirstResponder` + 复位回调）；`MenuBarLibraryScene` searchField 删手绘 HStack/`.textFieldStyle(.plain)`/清除按钮/`.quaternary` 背景/`@FocusState`，保留 `.onChange`→`setSearchQuery` 与排序 Picker；⌘F/深链契约不变；`LibrarySearchFieldSpec` 配置测试先行。
- [ ] T185 Sync schedule 语义 (FR-053 Rev 3): `AutoSyncPolicy.changeOnly.displayName` → "Off"（删除 "After changes only"）；`SyncSettingsView` automaticSyncSection 加 `Section("Automatic Sync")` 头、Toggle 改名 "Sync changes automatically"、Picker 改名 "Periodic sync"、`.disabled` 移除 `|| !isVaultUnlocked`（仅 `!autoSyncEnabled` 禁用）、锁定 caption 更新（含手动同步提示与本地可用条款）；`SyncSchedulePresentation.periodicPickerEnabled` 纯策略 + 测试；补 AutoSyncPolicy 5 条 displayName 的 xcstrings 双语条目（既有缺失）；Phase27 同步。
- [ ] T186 Sync 页 IA 重组 (FR-054 Rev 3): `SyncSettingsView` storageSection 增 "Manage…" `Menu`（Join Another Vault… → showJoinSheet、Set Up New Storage Location… → showReplaceSheet）；Recovery 行移入 securitySection；advancedContent 仅留 Vault ID + Export Sync Profile… + Export Diagnostic Bundle…；Disconnect Sync… 移出 Advanced 为表单底部独立 Section 破坏性入口（confirmationDialog 保留）；`SyncAdvancedAreaPolicy.operations` → ["Export Sync Profile","Export Diagnostic Bundle"]；`SyncCompositionTests` FR-054 断言重写；首配双入口不动。
- [ ] T187 Custom glass cleanup (FR-061/062): 先 context7 验证 macOS 26 `.buttonStyle(.glass)`/`GlassButtonStyle`；可用则 `RichTextBlockView` ContextualFormatBar 5 按钮与 `BlockInsertionControl` 插入按钮换系统 glass 按钮样式并移除 `regularMaterial + glassEffect` 叠层；不可用则仅去冗余 material 叠层；`#available(macOS 26, *)` 降级分支保留；`GlassUsagePolicy` 常量与测试不动。
- [ ] T188 菜单命令补全 (FR-072/SC-017): `MenuCommandCatalog.all` File 区增 Restore / Empty Trash… / Sync Now（无自定义快捷键）；`StickyNotesApp` CommandGroup(after: .newItem) 增对应 Button 与 handlers（`restoreFocusedNote` guard trash+keyboardSelection、`requestEmptyTrash` → 模型确认标志（与 T183 共用确认条）、`syncNow` → `SyncCoordinator.manualSync()`）；`MenuChecklistTests` 增存在性断言。
