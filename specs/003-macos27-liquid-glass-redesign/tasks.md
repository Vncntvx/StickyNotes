...
- [ ] T176 Unify Settings window path (FR-051 Rev 2): scene-first open via captured `@Environment(\.openSettings)` with runtime detection; delete manual-window creation only when the scene window is observed; keep documented NSWindow fallback for the macOS 27 beta LSUIElement case (`showSettingsWindow:` verified broken 2026-08-08); apply stable shell geometry: scene `.defaultSize(width: 640, height: 560)` + `.windowResizability(.contentMinSize)`, root `.frame(minWidth: 600, minHeight: 440, idealWidth: 640, idealHeight: 560)`; fallback window `minSize` 600×440 + `setFrameAutosaveName`; menu bar/app menu/⌘, all route through one entry (`CommandGroup(replacing: .appSettings)` + ⌘, shortcut).
