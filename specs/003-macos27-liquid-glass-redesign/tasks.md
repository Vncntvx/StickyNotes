...
- [ ] T178 Restructure SyncSettingsView (FR-053/FR-054 Rev 2): Status (resolver-driven) / Storage (truncating address) / Automatic Sync / Security ("Keep sync unlocked on this Mac" + verified Keychain copy) / Actions / Advanced (Join Another Vault…, Set Up New Storage Location…, Export Sync Profile…, Disconnect Sync…, Export Diagnostic Bundle…, Vault details, Recovery row); unconfigured state: Set Up Sync… / Join Existing Vault…; transient status/error feedback (auto-clear, `AccessibilityAnnouncements.announce`, no persistence across Settings reopen).
