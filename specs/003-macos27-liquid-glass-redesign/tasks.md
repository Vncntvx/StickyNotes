...
- [ ] T175 Update Settings policy sources + tests FIRST (Constitution XII): `App/Sources/Features/Settings/SettingsPolicies.swift` (`SettingsLayoutPolicy.logicalAreas` → `["General","Sync","Privacy"]`; remove `fixedCanvasHeight`; add `SettingsWindowPolicy` — defaultSize 640×560 / minimum 600×440 / `windowSizeStableAcrossTabs` / `onlyOverflowingTabsScroll`), `SyncAdvancedAreaPolicy.swift` (new operation names: Join Another Vault / Set Up New Storage Location / Disconnect Sync / Export Sync Profile / Export Diagnostic Bundle); update `AppTests/AppLogicTests.swift` (FR-050), `AppTests/SyncCompositionTests.swift` (FR-054), `AppTests/Phase27LocalizationTests.swift` (renamed strings); add `AppTests/SettingsWindowPolicyTests.swift`; suites MUST FAIL before implementation.
...
- [ ] T179 Sync state correctness (FR-053 Rev 2): `SyncCoordinator.isVaultUnlocked`; wire `SyncStatusResolver` with real locked state; disable Sync Now/frequency/remember-unlock when locked; minimal `unlock(password:)` (fetch bootstrap by locator → `openRemoteBootstrap` → `wireEngine`, read-only remote); Locked UI + Unlock… action; tests in `AppTests/SyncCoordinatorUnlockTests.swift` (in-memory provider: unlock success / wrong-password fail-closed).
