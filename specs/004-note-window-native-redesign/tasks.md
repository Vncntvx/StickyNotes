...
- [X] T009 [P] 新建 `AppTests/AppearancePanelStateTests.swift`：透明度 clamp 0.00–1.00/步 0.05（Q8 修订，原 0.40 下限取消）、"NN%" 数值格式（禁止 "10…"）、重置=默认色+1.0（spec FR-008/009，先红）
- [X] T010 [P] 新建 `AppTests/NoteToolbarStateTests.swift`：`toolbarVisibilityPriority` 映射（Pin=.high，其余 .standard，FR-015a）与工具栏项标识符固定集常量断言（FR-015c/Q3，先红）【2026-08-13 T065 修订：映射改为 Pin/Insert=.high、Appearance/More=.standard，布局常量 2+2】
...
- [X] T026 [P] [US3] 新建 `App/Sources/Features/NoteWindow/AppearancePanelView.swift`（SwiftUI）：7 键调色板（`NotePaletteKey` 含 custom，`paletteStorage` 映射逐字保留，选中=勾选+名称+色块）、透明度 Slider 0.00–1.00/0.05（Q8）、"NN%" 完整数值标签、恢复默认按钮（默认色+1.0）、无障碍 label/value（FR-008/009，001 FR-044）
- [X] T027 [US3] Appearance 工具栏项：`NoteToolbarController` 中 paintpalette 按钮 → 无边框子窗口面板（T071 替换 NSPopover 锚定，NSHostingController → `AppearancePanelView`）；改动 → `host.updateAppearance` + `coordinator.updateNotePaper`（即时预览，无确认按钮）；`menuFormRepresentation` = 外观子菜单（颜色 7 项 + 透明度 21 步（0–100%，Q8）+ 重置，自 NoteControlsView 上下文菜单迁移）
...
- [X] T063 长笔记滚动修复（2026-08-13 用户反馈"输入页面较长时无法向下滚动、无滚动条"）：根因 = 宽度感知 GeometryReader 包裹内容并接受 ScrollView 视口高度提议，长内容被裁剪在视口高度内；修复 = 宽度改经 `onGeometryChange` 测量（`paperWidth` @State），内容 VStack 自定尺寸；`NotePaperTextView` intrinsicContentSize 内 `ensureLayout` + `didChangeText` 重发 intrinsic（打字/模型推送均增长）；`EditorBlockEditingTests.notePaperTextViewIntrinsicHeightGrowsWithLongContent` 先红（usedRect 未布局）→ 实现转绿；滚动条跟随系统 overlay 显示
- [X] T064 工具栏视觉密度收减（2026-08-13 用户反馈"顶部 Glass toolbar 形态太重，像大型 segmented control"）：`NoteToolbarSpec` 新增密度常量 `toolbarSizeMode = .small` + `buttonControlSize = .small`（AppKit medium 密度带：总高约 -10–15%、单项水平 padding 约 -15–20%，玻璃 bezel 与 hover/pressed 形变保持系统提供，FR-022/FR-034 不手绘）；`NoteToolbarController` init 与 `makeToolbarButton` 接线；`NoteToolbarStateTests.toolbarUsesSmallSizeModeForMediumDensity` 先红（.regular≠.small）→ 转绿
- [X] T065 工具栏 2+2 语义分组与窄窗口溢出修正（2026-08-13 用户反馈"四联胶囊像四选一分段选择器；产品 … 被系统 » 收起形成两层菜单"）：布局改 `[pin, appearance] + 系统 .space + [insert, more]`（两个玻璃岛：便签属性组 vs 操作组，`NoteToolbarSpec.layoutIdentifierStrings`，delegate 接线）；优先级重映射 Pin/Insert=.high、Appearance/More=.standard（窄窗口保留两个主命令，外观/更多以子菜单形态进系统 »，消除两层"更多"）；`NoteToolbarStateTests` 重写优先级别断言 + 新增 2+2 布局断言（先红编译/断言 → 实现转绿）；spec FR-015/015a/015b、plan §3.1/§3.2/§3.3/§9.1、contracts §2、data-model §4.5 同步【2026-08-13 同日晚用户第三轮反馈：2+2 分组撤回（T067 单一 strip）；优先级重映射保留】
- [X] T066 外观 popover 锚定修正（2026-08-13 用户反馈"点击🎨时弹出的位置异常——popover 出现在屏幕顶部、箭头指向空白桌面"）：根因 = 直接以工具条按钮视图为锚（macOS 26/27 工具条条目视图可能位于翻转坐标容器，坐标被解析到错误空间）；修复 = `view.convert(view.bounds, to: nil)` 转窗口坐标，改锚 `window.contentView`（代码级修复 + 全量套件；GUI 观感人工验证）【2026-08-13 同日晚 T067b 再修：窗口基坐标矩形交给翻转的 NSHostingView 导致 Y 镜像（popover 出现在笔记中部、箭头朝下）——最终形态 = `view.convert(view.bounds, to: contentView)` 转入 contentView 自身坐标系后锚定，坐标空间与定位视图一致】【2026-08-13 同日晚 T068 第三次修：T067b 后仍出现在屏幕顶部远处（与 v1 相同）——判断 macOS 26/27 Liquid Glass 下工具条条目视图可能寄宿在私有窗口 shell 中，一切跨层级 convert 不可靠；改为纯屏幕坐标换算：按钮屏幕矩形（`(view.window ?? noteWindow).convertToScreen`）− 笔记窗口 contentView 屏幕矩形（`noteWindow.convertToScreen(contentView.frame)`）→ 手工构造 contentView 坐标系锚点矩形（含 isFlipped 分支与夹取），锚 `NoteWindowBridge.registeredWindow(for: noteId)` 的 contentView；若仍异常则 v5 兜底 = 手动定位的边框子窗口】
- [X] T067 工具栏最终形态：单一 Glass strip + 无边界 item（2026-08-13 用户第三轮反馈"2+2 像两根并排药丸；每项横向 ~55-60pt 太宽、胶囊边界太多、ellipsis.circle 圆形层级过多、置顶染色过重"）：撤回 2+2 `.space` 布局（单一 strip 固定集）；按钮 bezel 改 `.toolbar`（resting 无独立胶囊，hover/pressed/keyboard 系统局部响应，Glass 仅作整体容器）；symbol 显式点尺寸 14pt、palette 光学 -1pt=13pt；More 图标 `ellipsis.circle`→`ellipsis`；Pin 改 momentaryPushIn + 符号状态（pin ↔ pin.fill，不染色表面）；`NoteToolbarStateTests` 新增样式断言（先红编译 → 实现转绿）；spec FR-007、plan §3.1/§3.2/§3.3、contracts §2 同步
- [X] T068 外观 popover 屏幕坐标锚定（2026-08-13 用户反馈"再次出现在上方且离很远"）：判断 Liquid Glass 工具条条目视图寄宿私有窗口 shell，跨层级 convert 全部不可靠；改纯屏幕坐标换算（按钮屏幕矩形 − contentView 屏幕矩形 → 手工构造 contentView 坐标系锚点，含 isFlipped 分支与夹取），锚 `NoteWindowBridge.registeredWindow(for: noteId)` 的 contentView；用户验证落点正常（v4 生效，v5 手动子窗口兜底未启用）
- [X] T069 外观面板实时状态与菜单同步（2026-08-13 用户反馈"拖动不跟随鼠标、重开才见变化、菜单与正文不一致"）：根因 = 面板以不可变 `note` 快照驱动 Slider get（旋钮回弹、NN% 冻结）；修复 = 面板改本地 @State（transparency/colorKey/customColor）+ `NoteWindowDerivations.composeAppearance`（快照为基、当前态覆盖、非外观字段保留），Slider/色板/重置全部即时自刷新；`NoteToolbarController.refreshFromHost` 重建外观子菜单 menuFormRepresentation（溢出菜单勾选与面板/正文同步）；`AppearancePanelStateTests.appearanceCompositionCarriesLocalFieldsOverBaseNote` 先红 → 转绿
- [X] T069b ~~透明度范围显式 40%–100%（2026-08-13 用户指令"范围是 40%-100% 而不是 0-100%"）：Slider range/step 改接 `NoteAppearance.OpacityBounds` 常量（单一来源：面板/溢出菜单 13 步/⌥O 步进共用）；面板轨道下新增 "40%" / "100%" 端点标签（未标注轨道易读作 0–100% 连续体）；`AppearancePanelStateTests.opacityBoundsEnforceFortyPercentFloor` 钉住常量~~【2026-08-13 同日晚 Q8 反转：用户明确范围应为 0–100%——见 T070】
- [X] T070 透明度范围 0–100%（2026-08-13 Q8）：`NoteAppearance.OpacityBounds.minOpacity` 0.40→0.0（21 步）、`clamped` 改整数百分比运算（消除 0.30000000000000004 漂移，StickyCore 本轮首次改动）；App `clampedOpacity` 同步 0–100；面板端点标签改 "0%" / "100%"；测试先行：DomainTests（bounds/投影 0.30）、AppearancePanelStateTests（21 步/0 下限/钳制）、RegressionTests（旧 0.40 下限断言改写）先红 → 转绿；spec Q8/FR-008/025、plan §4.2/§3.2、contracts §3、data-model §1/§6、quickstart §3.3.2 同步
- [X] T071 外观面板改手动定位子窗口（2026-08-13 用户反馈"点击🎨又跑到上面去了"）：NSPopover 锚定在 macOS 27 Liquid Glass shell 下不可靠（四版锚定方案全部出现漂移）；改为无边框 `.nonactivatingPanel` 子窗口，位置由笔记窗口**屏幕 frame** 直接计算（`panelLeadingInset` 110 / `panelTopInset` 56，NoteToolbarSpec 常量），随父窗口移动/关闭；点击外部或 ESC 经本地事件监视器关闭；`appearancePopover` 属性改 `appearancePanel: NSWindow?`，teardown/deinit 同步清理
- [X] T072 透明度单源修复 + 面板实体材质（2026-08-13 用户反馈"标题栏条带透明度与正文不一致；面板被错误改成透明"）：① 根因 = 纸面层再画一层 color×alpha（`ReadableTheme.background`），叠加窗口背景（同样 color×alpha）后纸面合成 ≈ 1-(1-a)²，明显比条带更实——移除纸面填充，窗口背景成为唯一来源（`notePaperBackground` 仅保留圆角 clip）；② 面板（无边框子窗口、透明背景）加 `.regularMaterial` 圆角实体表面，不随笔记透明度变化；FR-025/FR-030a 注释同步
- [X] T073 ~~内容层透明度（2026-08-13 用户反馈"透明度 0 时字体会虚浮"）：根因 = 透明度只作用于纸面/窗口背景，文字以全不透明渲染，0% 时背景消失、文字残留；修复 = `NoteWindowContent` 内容分支末级加 `.opacity(note.transparency)`（research §1.4 语义：透明度 = 内容层 alpha，纸面与文字一起淡化；0% 时笔记含文字完全透明；原生镀铬不淡化）；spec FR-008 补透明度语义条款~~【2026-08-13 同日晚 Q9 反转：用户明确透明度只作用于背景、文字恒不透明、0% 时文字清晰可见——见 T074】
- [X] T074 透明度仅背景（2026-08-13 Q9，撤回 T073）：移除 `NoteWindowContent` 的 `.opacity(note.transparency)`；文字/内容保持全不透明，任何透明度（含 0%）清晰可读；0% 时纸面消失、文字完整浮于桌面；spec FR-008 语义条款改写（仅背景），plan §4.2 语义同步
