...
- [ ] T207 [US1] Implement device-local persistence of first-launch state in `App/Sources/Features/Library/LocalPreferences.swift` — store `onboardingHintSeen`, `onboardingHintDismissed`, `hasCreatedFirstNote` in App Group UserDefaults (device-local only; NEVER synchronized, NEVER in canonical JSON, NEVER in exported diagnostics per FR-191/data-model §LocalPreferences); set `hasCreatedFirstNote` on first-note creation; wire into `AppEnvironment` per FR-014a / research R28 / data-model §LocalPreferences / Constitution IV/VI
