...
- [X] T001 Create Xcode project via XcodeGen `project.yml` (source of truth) defining macOS app target `StickyNotes` and Widget Extension target `WidgetExtension` per plan.md §Project Structure; the generated `StickyNotes.xcodeproj` IS committed with regenerate-and-drift-check practice (`xcodegen generate` + commit, CI drift check) per T173/T249 and quickstart.md §Project generation
...
- [X] T025a Milestone 0 prototypes: SwiftUI rich-text + Chinese IME; Markdown single-Undo; one-window-per-note; per-window floating; App Group GRDB widget access; ScreenCaptureKit single-frame; native global shortcut; confirm Xcode 26.x/Swift 6.3 + integrate Argon2id per research.md R0–R18 in `Prototypes/` scratch directory outside the StickyCore package (no library/test target changes) — **fully verified 2026-08-07 (T158)**: headless prototypes (MarkdownUndo, AppGroupGRDB, GlobalShortcut criteria 1–3, Argon2id) PASS; GUI prototypes (RichTextIME, WindowCoordinator, ScreenCapture) PASS on a display-equipped Mac, launched as `.app` bundles (bare-binary `swift run` cannot activate windows/IME on macOS 27 beta — launch-mode limitation only). The Milestone 0 hard gate is COMPLETE.
...
- [X] T027 [P] [US1] Domain test: Note create/lifecycle + auto-discard empty note + preserve previously-content note when text empty per FR-012/FR-013 in `Packages/StickyCore/Tests/DomainTests/NoteLifecycleTests.swift`
...
- [X] T031 [P] [US1] Implement auto-save draft manager (debounce 500 ms per FR-141a — pinned value, superseded by T238; structural ops immediate, flush on focus-loss/close/terminate, revision tokens) in `Packages/StickyCore/Sources/EditorCore/AutoSave.swift`
...
- [X] T077 [P] [US6] Domain test: never-contained-content note auto-discardable on close; previously-content note NOT auto-deleted when text empty per FR-012/FR-013 in `Packages/StickyCore/Tests/DomainTests/EmptyNoteDiscardTests.swift`
...
- [X] T110 [US9] Domain test: remote object names opaque/random; no semantic type in filenames; manifest carries only opaque names+sizes/times per FR-160b/FR-161 in `Packages/StickyCore/Tests/DomainTests/RemoteLayoutTests.swift`
...
- [X] T158 Interactive verification of Milestone 0 GUI prototypes (T025a gate remainder) — **RESOLVED 2026-08-07**: ran `RichTextIMEPrototype`, `WindowCoordinatorPrototype`, `ScreenCapturePrototype` on a Mac with a display (macOS 27 beta, Xcode-beta 27.0). Finding: `swift run`/bare-binary launch cannot activate the window or focus the IME on macOS 27 beta (keyboard events stay in the launching app) — a launch-mode limitation, NOT a product defect; packaging each prototype as a minimal `.app` bundle (Info.plist + `open`) fixes it, matching production launch via LaunchServices. All three PASS with the bundle launch: RichTextIMEPrototype — Chinese IME typing works in the SwiftUI `TextEditor` (marked-text composition intact) and Round-trip check reports `PASS: lossless round-trip (NFC), 1 paragraphs, 149 JSON bytes`; WindowCoordinatorPrototype — multiple windows, one-window-per-note invariant (`windows count: 1 (invariant)` for Notes A/B), focus-to-existing-not-duplicate, Always-on-Top floating level all work; ScreenCapturePrototype — system content-sharing picker (window/all-windows) works, the screen-recording permission prompt appears ONLY on first capture invocation (FR-131), and region drag-capture succeeds (`region captured`). T025a's partial note is flipped to fully verified.

---

## Phase 15: Convergence

> **Note**: Tasks T159-T172 supersede and consolidate the corresponding Phase 3-12 tasks (T032-T119), whose `[X]` markers record their convergence re-execution via the T159-T172 audit trail rather than an initial passing state. The Phase 3-12 originals are retained for historical traceability; the convergence tasks are authoritative and should be executed in their place.
...
- [X] T194 [P] [US9] SecurityCore test: verify meaningful-metadata positive enumeration per FR-160a/FR-161 per FR-160a in `Packages/StickyCore/Tests/SecurityCoreTests/MeaningfulMetadataEnumerationTests.swift` (详情见 history/tasks-log.md)
...
- [X] T217 [US6] Pin FR-012a meaningful-text threshold in auto-discard logic per FR-012a/FR-013 in `Packages/StickyCore/Sources/Domain/NoteLifecycle.swift` (详情见 history/tasks-log.md)
...
## Phase 26: Convergence — 2026-08-07 App-Layer Wiring Audit

**Purpose**: Close the gaps found by the 2026-08-07 converge audit of the App layer. The StickyCore modules (Domain/Persistence/EditorCore/AssetStore/SecurityCore/SyncCore/SystemBridge) are implemented and green (578 StickyCore + 25 AppTests pass), but the App UI layer is a render-only shell: editor/appearance edits never persist, FR-031 contextual actions are dead stubs, library search does not consume the FTS SearchService, sync status/manual sync are hardwired to "not configured", the US9 sync surface (vault bootstrap/Keychain/SyncEngine) is unwired, and several FRs (FR-001a, FR-002a, FR-032/FR-033, FR-004 Help) are implemented only as helpers or not at all. Tests written FIRST and must FAIL before implementation (Constitution XII). Phase 26 depends on: T030 (NoteRepository), T031/T238 (AutoSave), T042 (SearchService), T160 (NoteWindowCoordinator), T164 (LibrarySearchView), T170 (SyncSettingsView), T171 (SyncEngine), T112 (VaultBootstrap), T255 (FR-001a helper), T233 (NoteExportImport). Additive; blocks nothing in Phases 3–25.

- [X] T281 [US1] [US3] Wire editor + appearance persistence in the note-window host — in `App/Sources/Features/NoteWindow/NoteWindowCoordinator.swift` (`NoteWindowContent`) persist every `onChanged`/`onBlocksChanged` update through `NoteRepository` (note appearance: title/color/opacity/textSize/alwaysOnTop; block payloads: rich-text/todo/code/fileRef/image/screenshot) using the EditorCore AutoSave debounce (500 ms, single transaction per FR-141a) decoupled from the sync debounce; flush synchronously before window close/delete/quit; wire the FR-012a auto-discard decision (never-content note MAY be auto-removed, previously-content note MUST NOT) via an `NSWindowDelegate.windowWillClose` hook that also flushes pending edits; assert via a failing-first test: type text → close → reopen → content preserved (US1 AC3), appearance changes survive relaunch (FR-031/FR-032/FR-034), crash-loss ≤ one debounce window (FR-141a, extends T229) per US1/AC3, FR-006/FR-141/FR-141a/FR-012a / Constitution III/X/XII (partial) — **the current host holds edits only in SwiftUI `@State`; nothing reaches the database**
- [X] T282 [US1] [US3] Wire the FR-031 note-level contextual actions — replace the empty stubs in `App/Sources/Features/NoteWindow/NoteControlsView.swift` (onDuplicate/onCopyAsMarkdown/onExport/onMoveToTrash) with calls into `NoteExportImport` (export via NSSavePanel per FR-031a, copy-as-Markdown, duplicate with fresh sort key per FR-022a) and the library/coordinator move-to-Trash path (closes window + FR-009a toast); persist the FR-112 widget-eligibility toggle via `onChanged` → repository update; assert via failing-first tests (extends T245/T247/T276): duplicate yields byte-identical blocks with new UUID, copy-as-Markdown round-trips text, export/import round-trip, eligibility toggle persists across relaunch per FR-031/FR-031a/FR-112 / Constitution V/X/XII (partial) — **the menu renders but every action is a no-op; reconciles T248 (Phase 22): T248 delivered the duplicate/copy-as-Markdown helpers in NoteExportImport, this task wires the NoteControlsView stubs to them**
- [X] T283 [US2] Route library search through the FTS `SearchService` — in `App/Sources/Features/Library/LibraryModel.swift` replace the in-memory title/summary/previewSource filter with the FTS5-backed query (SearchService, T042) so search matches manual titles, body text, todo text, code-block text, file display names, and screenshot captions (FR-023), keeps FR-024a prompt updates (≤100 ms, incremental) and privacy exclusion, and is NOT bounded by `CardProjection.maxRows` (500) — matching notes beyond the first 500 cards must appear; assert via failing-first tests (extends T038/T163p): a search term present only in todo text/code/filename/caption matches; a match beyond the 500-card bound appears; 10k-note query completes within SC-005's 200 ms on the app path per FR-023/FR-023a/FR-024a/SC-005 / Constitution XI/XII (partial) — **the app path never consumes SearchService; only the first 500 cards are searched**
- [X] T284 [US9] Wire real sync state into the menu-bar library — in `App/Sources/Features/Library/MenuBarLibraryScene.swift` replace the hardcoded `SyncStatusView(isConfigured: false, manualSync: {})` with the actual `VaultConfiguration` presence, last-successful-sync time, actionable error, and in-progress state (per FR-151), so FR-014a's "not configured" shows only when sync is genuinely unconfigured and manual sync is triggerable from the library (FR-004); assert via a failing-first test: with a configured vault the library shows status/error + enabled manual sync, never "not configured" per FR-004/FR-014a/FR-151 / Constitution III/XII (partial) — **the status area is hardwired off regardless of configuration**
- [X] T285 [US9] Implement the app-side sync composition — in `App/Sources/App/AppEnvironment.swift` (SyncServices empty shell) + `App/Sources/Features/Settings/SyncSettingsView.swift` (static labels; Configure/Replace buttons empty; Manual Sync disabled) wire VaultBootstrap (T112) + Keychain + SyncEngine (T117/T198) into the environment; make SyncSettingsView functional: configure/test repository (WebDAV/S3 per FR-150), enable/disable automatic sync, manual sync with non-blocking FR-141b status, last-success time, actionable errors, remove local config without deleting local notes (FR-151), repository replacement with warning + fresh bootstrap + no auto-delete of prior remote data (FR-154), unrecoverable-password warning (FR-163), and the FR-162a remember-unlock toggle; startup/periodic/network-restore/termination sync triggers per FR-152; assert via failing-first tests (extends T178/T163h): end-to-end configure→manual sync→status, replacement flow, toggle semantics per US9/AC1,AC4, FR-150/FR-151/FR-152/FR-154/FR-162/FR-162a/FR-163 / Constitution VII/VIII/XII (missing) — **the entire US9 app surface is a static placeholder; StickyCore sync is tested but never composed into the app**
- [X] T286 [US1] Apply the FR-001a library-window frame at presentation — invoke `MenuBarLibraryWindow.positionLibraryWindow` (`App/Sources/Features/Library/MenuBarLibraryScene.swift`) when the MenuBarExtra window appears (e.g. via an `NSViewRepresentable` window-probe or `MenuBarExtra` window appearance hook) so the library is left-edge-aligned with the status-item icon, clamped to the visible screen frame, 4 pt below the menu bar, with no animation; assert via a failing-first UI-level test that the presented window's frame matches `MenuBarWindowFrame.libraryWindowFrame` (extends T250) per FR-001a / research R40 / Constitution XI/XII (partial) — **the helper exists but is never called; the window is positioned by the system**
- [X] T287 [US2] Implement the FR-002a card-grid metrics — in `App/Sources/Features/Library/LibrarySearchView.swift` (`LibraryCardGrid`) make the grid default to 3 columns with ~220×160 pt cards and 12 pt inter-card spacing, reducing to 2 columns below 600 pt and 1 below 400 pt window width (replacing `GridItem(.adaptive(minimum: 220, maximum: 340), spacing: 10)` + `minHeight: 96` in `NoteCardView.swift`); assert via a failing-first rendering test (extends T251) per FR-002a / Constitution XI/XII (partial) — **the fixed 420 pt library width yields a single column and 10 pt spacing; card height is unbounded**
- [X] T288 [US1] Add the Help affordance to the menu-bar library — in `App/Sources/Features/Library/MenuBarLibraryScene.swift` add a Help button (footer, alongside About/Settings) opening a localized Help view (keyboard shortcuts, auto-save, menu-bar-primary model, per FR-052a documentation), so Settings/Help/About/sync status/Quit are all reachable from the menu-bar interface when the Dock icon is disabled (FR-008); assert via a failing-first test (extends T149): with Dock in accessory policy, Help is reachable and opens per FR-004/FR-008 / Constitution X/XII (partial) — **the footer lacks Help entirely**
- [X] T289 [US3] Wire window frame persistence + display-change handling — in `App/Sources/Features/NoteWindow/NoteWindowCoordinator.swift` save/restore each note's window frame + preferred display via `WindowStateRepository` (device-local, never synced) so the last size/position is remembered (FR-032), and apply `DisplayChangeBridge` (T054) so a window on a disconnected display moves into the visible area while its preferred frame is preserved for reconnect (FR-033); wire the `windowWillClose` hook (shared with T281) to save the frame; assert via failing-first tests (extends T047/T048): frame persists across close/reopen, disconnected-display frame correction + restoration per FR-032/FR-033 / Constitution X/XII (partial) — **windows are created at a fixed 420×480 @ (200,200) and WindowStateRepository/DisplayChangeBridge are never consumed**

## Phase 27: Convergence — 2026-08-07 Block-Editing & Capture-Wiring Audit

**Purpose**: Close the gaps found by the post-Phase-26 converge audit. Phase 26 wired persistence, search, sync, and window behavior, but the editor's block-editing surface and the capture/asset pipeline remain unwired: users cannot insert todo/code/file/screenshot/image blocks, todo completion is never persisted (TodoRepository unused), file-reference card actions are dead buttons with a hardcoded `.available` indicator, screenshot covers/captions never persist and the viewer never opens, the capture pipeline (RegionCapture/WindowCapture/AssetStore) has no App composition, widget refresh (FR-110a) has no callers, new UI strings are missing from the localization catalogs, and global-shortcut configuration is static. Tests written FIRST and must FAIL before implementation (Constitution XII). Phase 27 depends on: T030 (NoteRepository/TodoRepository), T061 (TodoRepository), T064 (FileReference/FileLocator), T087 (AssetStore), T089/T090 (WindowCapture/RegionCapture), T092/T168 (ScreenshotViewer), T237 (WidgetRefreshCoordinator), T239 (localization catalogs), T102 (GlobalShortcuts), T166 (block views). Additive; blocks nothing in Phases 3–26.

- [X] T290 [US4] Implement the editor block-editing surface — (a) block insertion: keyboard/menu affordances in `App/Sources/Features/Editor/RichTextBlockView.swift` (+ `NoteWindowCoordinator.NoteWindowContent`) to insert rich-text/todo/code blocks (FR-050: todos, multiline code blocks; FR-080) and a Finder drag-drop (`onDrop`) to insert a file-reference block per FR-100; (b) todo editing: `TodoBlockView` must persist completion through `TodoRepository.setComplete` (FR-070/FR-071 stable identity; `AppEnvironment.persistence.todoRepository` exists but is never consumed), and provide drag-reorder (`TodoRepository.reorder`), indent/outdent with depth bound 6 (`reparent`, FR-072a), edit, and delete (`delete`, children reparent to grandparent); assert via failing-first tests (extends T056/T060/T163f): insert a todo → toggle → reopen → completion persisted; reorder/indent persist; depth-6 bound enforced; a Finder file dragged onto the note window creates a file-reference block per US4/FR-050/FR-070/FR-071/FR-072a/FR-080/FR-100 / Constitution V/XII (partial) — **no insertion UI exists; the toggle flips local `@State` only**
- [X] T291 [US4] Wire the file-reference card actions + four-state availability — in `App/Sources/Features/Editor/CodeBlockView.swift` (`FileReferenceCardView`): implement open/reveal/copy-path via `SecurityScopedBookmarks` (balanced start/stop, T166), drag-out via `FileDragOutBridge` (copy, never move — FR-102; explicit move with destination + confirmation), relink (FR-103, missing-file preserves the card), and remove; evaluate `FileAvailability` from the bookmark state so the FR-100 indicator distinguishes available/missing/stale/on-another-device by icon + text (FR-044); assert via failing-first tests (extends T059/T163e/T275): each action resolves through the bookmark bridge, availability transitions persist, no filesystem scan (FR-103) per FR-100/FR-101/FR-102/FR-103/FR-104/FR-105 / Constitution IX/XII (partial) — **open/relink buttons are empty and `availability` is hardcoded `.available`**
- [X] T292 [US7] Persist screenshot cover/caption and open the viewer — in `App/Sources/Features/Editor/ScreenshotBlockView.swift`: persist cover selection transactionally (at most one cover per note — `Note.coverScreenshotBlockId`, FK nullified on delete per FR-094b) and caption edits through the host/repository; wire the FR-095a viewer (`App/Sources/Features/Capture/ScreenshotViewer.swift` — zoom 25–400%, arrow navigation, Return/caption-edit) so clicking a screenshot opens it in an independent note-style window that never activates the captured app; give `EmbeddedImageBlockView` its FR-090 actions (view/larger/copy/drag-out/save-elsewhere/remove); assert via failing-first tests (extends T084/T086/T163g/T253/T260): cover persists across relaunch, deleting the cover block nullifies the reference in the same transaction, viewer zoom/navigation contract holds per FR-090/FR-093/FR-094/FR-094b/FR-095/FR-095a / Constitution IV/V/XII (partial) — **cover/caption are local `@State`; the viewer has no callers; the embedded-image block has no actions**
- [X] T293 [US7] Compose the capture + asset pipeline into the App — in `App/Sources/App/AppEnvironment.swift`: give `AssetServices` a real storage root (App Group container subdirectory) and compose `AssetStore` (T087); add FR-031 upper-area "add screenshot / add file reference" buttons to `NoteControlsView` (P1 surface) and wire `RegionCapture`/`WindowCapture` (T089/T090, SystemBridge — permission prompted only on invocation per FR-131) so region/window captures insert a screenshot block into the current/new note; pass the composed AssetStore into `SyncCoordinator`'s SyncEngine so assets sync as independent encrypted objects (FR-090a, currently `assetStore: nil`); assert via failing-first tests (extends T085/T163h): a captured frame is stored + associated transactionally, asset limits apply (FR-090b), and the capture loop works end-to-end per US7/FR-031/FR-090a/FR-091/FR-131 / Constitution VI/XII (missing) — **no App callers of RegionCapture/WindowCapture; `AssetServices.directoryURL` is nil; the sync engine runs with `assetStore: nil`**
- [X] T294 [US8] Trigger change-driven widget refresh — call `WidgetRefreshCoordinator.refresh(for:…)` from every widget-affecting write: note created/edited/deleted/trashed/restored (LibraryModel + NoteWindowHostModel), todo toggled (T290), widget-eligibility changed (T281 host appearance path), conflict copy created; assert via failing-first tests (extends T228): each write reloads exactly the affected widget kinds and no fixed polling timer exists per FR-110a / Constitution VI/XI (missing) — **`WidgetRefreshCoordinator` has zero callers**
- [X] T295 [US1] [US9] Add zh-Hans + en translations for the Phase 26 UI — populate `App/Resources/Localizable.xcstrings` with the 16+ missing keys (Help view topics + actions, sync settings status/buttons/errors, "Moved to Trash"/"Permanently Deleted" toasts, "Sync not configured", "Sync Now", "Syncing…", configure/replace sheet fields) in both zh-Hans and en; assert via failing-first tests (extends T230): every new key resolves in both languages and the source scan finds no new hard-coded UI strings per FR-180a / Constitution X/II (partial) — **new strings fall back to English under zh-Hans**
- [X] T296 [US8] Implement global-shortcut configuration — in `App/Sources/Features/Settings/SettingsView.swift` replace the static `⌘⌥⇧N` label with a functional configuration surface (per-shortcut recorder) for the FR-120 set (library toggle, blank note, region capture, window capture, clipboard note, search, show/hide note windows) using `GlobalShortcuts` (T102, conflict detection + re-register per FR-121); register the additional shortcuts in `StickyNotesApp.wireGlobalShortcuts`; assert via failing-first tests (extends T150): each configured shortcut fires while another app is focused, conflicts surface non-blockingly per FR-120/FR-121 / Constitution X/XII (partial) — **only the clipboard-note shortcut is registered; the Settings label is static text**

## Phase 28: Convergence — 2026-08-07 Viewer, Window-Chrome & Wiring Audit

**Purpose**: Close the gaps found by the post-Phase-27 converge audit. Phase 27 wired block editing, file actions, capture, widget refresh, localization, and shortcuts; remaining items: the FR-095 screenshot viewer renders a placeholder (no AssetStore image load; no copy/drag-out/Save-As/delete-association), FR-030a note-window chrome is not applied (titled style, no border/radius/shadow), interpolated user-visible strings fall back to English under zh-Hans (FR-180a), the EditorCore FR-050a/FR-054 operations are never wired into the App editor, hover-only upper-area controls lack keyboard alternatives (FR-181), and conflict-copy creation during sync doesn't refresh widgets (FR-110a). Tests written FIRST and must FAIL before implementation (Constitution XII). Phase 28 depends on: T087 (AssetStore), T092/T168 (ScreenshotViewer), T160 (NoteWindowCoordinator), T235 (BlockMergeOperation), T259 (CrossBlockSelectionCore), T239 (catalogs), T171 (sync conflict path), T294 (widget refresh). Additive; blocks nothing in Phases 3–27.

- [X] T297 [US7] Load the real screenshot image in the FR-095 viewer and complete its action set — in `App/Sources/Features/Capture/ScreenshotViewer.swift` + `MediaPresenters.swift`: load the selected screenshot's thumbnail (FR-094a 256px, never full-res in grids) via the AssetStore (inject a `(UUID) async throws -> Data?` image provider; the presenter wires the composed `AssetServices.store`), display the actual image (original at ≥100% zoom, thumbnail below per FR-094a), and add the missing FR-095 actions: Copy (pasteboard PNG), Drag-out (copy-only, FR-102 semantics), Save As (NSSavePanel), and Delete Association (removes the block + cascades, FR-094b cover nullification); keep zoom 25–400% steps, arrow navigation, and Return caption-edit (already implemented and T253-tested); assert via failing-first tests (extends T253/T163g): the viewer resolves asset bytes for a stored fixture, copy puts the PNG on the pasteboard, delete-association removes the block and nullifies `coverScreenshotBlockId` per FR-095/FR-095a/FR-094b / Constitution IV/V/XII (partial) — **the viewer shows a camera placeholder and none of the four actions exist**
- [X] T298 [US3] Apply the FR-030a note-window chrome — in `App/Sources/Features/NoteWindow/NoteWindowCoordinator.swift` (window creation) apply the note-paper appearance: borderless or thin-title-bar (`titlebarAppearsTransparent` + `titleVisibility = .hidden` on the `.titled` window, or `.borderless` with a custom title bar per FR-031's integrated upper area), a 1-pt subtle border (window `borderWidth`/layer stroke or an overlay), 8-pt corner radius, and a soft shadow (radius 8, opacity 0.15, `hasShadow = true`); the upper control area must stay visually integrated with the note background (no distinct toolbar chrome); assert via failing-first tests (extends T049/T046): the window's style/level/shadows meet the FR-030a contract and always-on-top still floats per FR-034 per FR-030a / Constitution X/XI (partial) — **the window is a plain `.titled` window with default chrome**
- [X] T299 [US1] [US9] Localize interpolated user-visible strings — add the interpolated keys to `App/Resources/Localizable.xcstrings` with `%@`/`%lld` placeholders and zh-Hans + en translations, and switch call sites to the formatted form: relative time ("%lld min ago" / "%lld hr ago" / "%lld days ago" — `App/Sources/Features/Shared/Formatters.swift:33-40`), "Synced %@" (`SyncStatusView.swift:63`), "Screenshot %lld of %lld" (`ScreenshotViewer.swift:53`), "Connection failed: %@." and "Configuration failed: %@." (`SyncSettingsView.swift:344,378`); assert via failing-first tests (extends T230/T295): each interpolated key exists with both languages and a zh-Hans user sees the translated template per FR-180a / Constitution X/II (partial) — **interpolated strings render English under zh-Hans**
- [X] T300 [US1] [US5] Wire the FR-050a empty-block removal + FR-054 cross-block selection into the App editor — in `App/Sources/Features/Editor/RichTextBlockView.swift` (single rich-text block surface): (a) on cursor exit from an emptied block/paragraph, invoke `BlockMergeOperation` (T235) so the emptied block merges away with a single Undo, never removing the final block, suppressed during IME composition (FR-063); (b) route selection operations through `CrossBlockSelectionCore` (T259) so copying a spanning selection places plain + rich text with supported formatting only and range-delete merges emptied blocks; assert via failing-first tests (extends T226/T254/T235): the App path triggers the same operations the EditorCore tests verify per FR-050a/FR-054 / Constitution V/XII (partial) — **both EditorCore components are implemented and tested but never called by the App**
- [X] T301 [US3] Add keyboard alternatives for the hover-only upper-area controls — in `App/Sources/Features/NoteWindow/NoteControlsView.swift` make the color/opacity/text-size controls reachable without pointer hover (FR-181): e.g. keyboard shortcuts (⌥C color, ⌥O opacity, ⌥T text size) and/or a keyboard-accessible menu (the contextual menu already hosts note-level actions — extend it with appearance items); the controls remain hidden until hover for mouse users (FR-031) but the actions stay possible via keyboard; assert via failing-first tests (extends T131/T267): each appearance action is invocable without hover per FR-181 / Constitution X (partial) — **color/opacity/text-size are hover-only today**
- [X] T302 [US9] Refresh widgets when sync creates conflict copies — in `App/Sources/App/SyncCoordinator.swift` (runSync): after a sync pass whose summary indicates new conflict copies (extend `SyncSummary` with a `conflictCopiesCreated: Int` count in `Packages/StickyCore/Sources/SyncCore/SyncEngine.swift`, populated by the conflict resolver path), call `WidgetRefreshCoordinator.reload(for: .conflictCopyCreated)` so the affected kinds (FR-110a) refresh; assert via failing-first tests (extends T228/T294): a sync pass creating a conflict copy triggers exactly the conflict-copy kinds per FR-110a / Constitution VI/XI (partial) — **conflict-copy creation is invisible to the widget layer**
