...
- Q: When the user searches and no notes match, or opens Trash while it is empty, what should the UI show? → A: A unified empty-state component — one reusable, localized placeholder (message + icon, no call-to-action) used for both search no-results and empty Trash (FR-014c).
- Q: When the user clicks the menu-bar icon while a modal sheet (Settings, Save As, export dialog) is already open, what happens? → A: The sheet stays open and the library toggles normally per FR-009 — the app-modal sheet is never silently dismissed by the menu-bar toggle (FR-009; Edge Cases).
- Q: How should the editor present each block category visually (rich text, todo, code, file-reference, embedded image, screenshot)? → A: A unified block container style consistent with the FR-030a note-window aesthetic, plus per-category distinguishing affordances (todo checkbox, code font, file card layout, image frame); no full per-category pixel specs (FR-050b).
...
- The menu-bar icon is clicked while an app-modal sheet (Settings, Save As,
  export dialog) is open: the sheet stays open and the library toggles
  normally; the sheet is never dismissed by the toggle (FR-009).
...
- **FR-009**: Clicking the menu-bar icon while the library window is already
  ...
  window MUST NEVER be opened. When an app-modal sheet (Settings, Save As,
  export dialog, or any other sheet of the application) is open at the moment
  of the click, the sheet MUST remain open and the library MUST toggle exactly
  as specified above — the menu-bar toggle MUST NOT dismiss an open sheet.
...
- **FR-011a**: Exception behavior for the library and search surfaces follows
  a general resilience guarantee: a failure in any library action (open
  note window, toggle/dismiss library, switch sort, manual reorder, search)
  MUST NOT crash the application, MUST NOT lose or corrupt note data, and
  MUST surface as a non-blocking, localized status message when the failure
  is user-visible (FR-141b/FR-191 sanitization applies). The two
  highest-impact cases have explicit rules: (a) if a note window fails to
  open (e.g., window creation error), the library MUST remain usable, the
  failure MUST be reported non-blockingly, and retrying the open action is
  permitted; (b) search returning no results MUST render the unified
  empty-state component (FR-014c) and MUST NOT be treated as an error.
  Sort-switch and manual-reorder failures need no per-case rules: they fall
  under the general guarantee plus FR-022a's transactional persistence
  (a failed reorder never leaves a partial order observable).
...
- **FR-014c**: Search no-results and empty-Trash MUST both use a unified
  empty-state component: a single reusable placeholder (localized message +
  icon, no call-to-action, no other content) presented in the same
  component, spacing, and visual style on both surfaces, localized per
  FR-180a. The first-launch empty-library state (FR-014a) is a distinct
  richer variant with its CTA and onboarding hint and MUST NOT be
  substituted for the unified empty-state.
...
- **FR-050b**: Block presentation in the editor MUST use one unified block
  container style consistent with the note-window aesthetic (FR-030a: same
  corner radius family, no per-block borders or backgrounds by default,
  consistent vertical spacing between blocks). Each block category MUST be
  visually distinguishable by its inherent affordances only: todo rows show a
  checkbox, code blocks use the monospaced font family (FR-080), file
  references render as a compact card with the FR-100 fields, embedded images
  and screenshots render as framed media with the FR-094a thumbnail (scaled,
  not decoded at full resolution). Per-category spacing, border, or
  background values beyond these affordances are implementation choices and
  MUST NOT be specified as pixel requirements.
...
- **FR-094b**: When a screenshot block that is currently selected as the note
  ...
  `DEFERRABLE INITIALLY DEFERRED`) AND reinforced by the
...
- **FR-141b**: Visual feedback for asynchronous operations MUST follow a
  split policy. Background operations — automatic saving (FR-141a), search
  query updates (FR-024a), and thumbnail generation (FR-094a) — MUST be
  silent: no progress indicator, spinner, or completion toast is shown, and
  the interface remains fully interactive (FR-153). User-initiated
  operations — screenshot capture (FR-091), manual synchronization
  (FR-151), and single-note JSON export/import (FR-031a) — MUST provide
  explicit, non-blocking status feedback (status text or indicator in the
  relevant surface, localized per FR-180a) so the user can tell the
  operation is in progress and when it finishes or fails. No async
  operation MAY block typing or other interaction (FR-153).
...
- **FR-180b**: Accessibility labeling follows a scoped policy. Standard
  platform controls (buttons, menus, text fields, sliders, toggles, table
  rows, toolbar items) MUST rely on platform-provided accessibility labels
  and actions; the application MUST NOT add custom labels that duplicate
  visible text. Custom-built controls MUST provide explicit, localized
  labels and actions (FR-180a): the file-reference card (open, reveal,
  copy path, relink, move, remove per FR-101), the screenshot viewer
  (zoom, actual size, fit-to-window, copy, drag out, Save As, delete
  association, edit caption, navigation per FR-095a), the upper-area
  hover controls (FR-031), and editor block affordances (todo checkbox
  with completed/incomplete state per FR-070/FR-182, code-block copy
  button per FR-081, image/screenshot blocks). Required VoiceOver
  announcements: the deletion toast (FR-009a) and completion of
  user-initiated operations with explicit status feedback (screenshot
  capture finished/failed, manual sync finished/failed, JSON
  export/import finished/failed per FR-141b).
