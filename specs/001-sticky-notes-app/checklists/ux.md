...
- [X] CHK014 - Are requirements specified for the empty-state behavior of every surface — empty library, empty search results, empty Trash, no notes eligible for widgets, no sync configured? — RESOLVED: FR-014a covers empty-library (CTA, onboarding hint) and sync "not configured"; FR-014c specifies the unified empty-state component for search no-results and empty Trash; FR-112 now specifies the no-eligible-note widget fallback (FR-140a "temporarily unavailable" placeholder) [Completeness, Spec §FR-014a/FR-014b/FR-014c/FR-024/FR-112/FR-140a]
...
- [X] CHK095 - Are requirements specified for the widget-eligibility toggle UX — where in the note UI is the setting, is it on the upper-area control bar, and is it keyboard-accessible? — RESOLVED: FR-112 now specifies a note-level toggle in the contextual menu (with the FR-031 note-level actions), keyboard-accessible per FR-181, not on the upper-area control bar [Completeness, Spec §FR-112/FR-031]
